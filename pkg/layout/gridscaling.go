...
func (layout *GridScalingLayout) Layout(objects []fyne.CanvasObject, size fyne.Size) {
	...
	for index, object := range objects {
		...
		object.Move(fyne.NewPos(theme.InnerPadding()+float32(index%layout.columns)*(objectwidth+theme.InnerPadding()), float32(index/layout.columns)*(objectheight+theme.InnerPadding())))
	}
}
...
func (layout *GridScalingLayout) MinSize(objects []fyne.CanvasObject) fyne.Size {
	...
	minheight := objectsInY*objectheight + (objectsInY-1)*theme.InnerPadding()
