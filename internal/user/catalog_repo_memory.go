...
func (repo *InMemoryCatalogRepository) ListUsers(ctx context.Context) ([]CatalogItem, error) {
	repo.mu.RLock()
	defer repo.mu.RUnlock()
...
func (repo *InMemoryCatalogRepository) GetByIP(ctx context.Context, ip string) (CatalogItem, error) {
	repo.mu.RLock()
	defer repo.mu.RUnlock()

	return repo.catalog[ip], nil
}
